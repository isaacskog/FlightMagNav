...
settings.tune_hyper_par=false;                                              % Turn on optmization of hyperparameters (in this case the measurement noise variance)
...
settings.map.length_scale =-2*pi*50/log(0.01);                              % Lengtyh scale used in the basis functions [m]
settings.time.sigma_q = 0;                                                  % Random walk noise [nT/s] for modeling temporal variations
settings.time.sigma_g0 = 30;                                                % Prior uncertianty (std) on the temporal variation bias [nT]. This also captures total changes in the field level between the flights. 
...
settings.calibration.sigma_ori = 1e-4;                                      % Prior uncertianty (std) on the orientation dependent biases [-]
settings.calibration.sigma_back_bias = 1;                                   % Prior uncertianty (std) on the biases between front and back sensor [nT]
